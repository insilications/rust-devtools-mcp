...
use rmcp::{
    ...
    transport::{stdio, streamable_http_server, SseServer},
    ServiceExt,
};
use server::DevToolsServer;

use crate::{context::Context, project::TransportType};
...
pub async fn run_server(context: Context) -> Result<()> {
    ...
    match context.transport() {
        ...
        TransportType::Sse {
            ...
            let cancel_token = sse_server.with_service(move || DevToolsServer::new(context.clone()));
